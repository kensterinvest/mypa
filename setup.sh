...
# ntfy from official .deb if not present
NTFY_VERSION="${NTFY_VERSION:-2.11.0}"
if ! command -v ntfy >/dev/null 2>&1; then
    say "Installing ntfy ${NTFY_VERSION}…"
    NTFY_ARCH=$(dpkg --print-architecture)
    curl -fsSL "https://github.com/binwiederhier/ntfy/releases/download/v${NTFY_VERSION}/ntfy_${NTFY_VERSION}_linux_${NTFY_ARCH}.deb" \
        -o /tmp/ntfy.deb
    dpkg -i /tmp/ntfy.deb >/dev/null
    rm /tmp/ntfy.deb
else
    say "ntfy already installed (skipping)."
fi
...
# ---------- 6b. ntfy server + auth ----------
say "Configuring ntfy server (auth-enabled)…"
NTFY_HOST="${E[NTFY_HOST]:-}"
if [[ -z "$NTFY_HOST" ]]; then
    # Default to ntfy.<root domain of PUBLIC_HOST>
    PUBLIC_ROOT="${E[PUBLIC_HOST]#*.}"
    read -r -p "  ntfy hostname [ntfy.${PUBLIC_ROOT}]: " NTFY_HOST
    NTFY_HOST="${NTFY_HOST:-ntfy.${PUBLIC_ROOT}}"
    E[NTFY_HOST]="$NTFY_HOST"
fi

# Generate publisher + admin creds if not already in env
if [[ -z "${E[NTFY_PUBLISH_PASSWORD]:-}" ]]; then
    E[NTFY_PUBLISH_USER]="mypa-publisher"
    E[NTFY_PUBLISH_PASSWORD]=$(gen_secret 32)
fi
if [[ -z "${E[NTFY_ADMIN_PASSWORD]:-}" ]]; then
    E[NTFY_ADMIN_USER]="admin"
    E[NTFY_ADMIN_PASSWORD]=$(gen_secret 32)
fi
E[NTFY_BASE_URL]="https://${NTFY_HOST}"

install -d -o ntfy -g ntfy -m 0750 /var/cache/ntfy /var/lib/ntfy 2>/dev/null || \
    install -d -m 0750 /var/cache/ntfy /var/lib/ntfy

cat > /etc/ntfy/server.yml <<EOF
# Generated by mypa setup.sh — auth enabled.
base-url: "https://${NTFY_HOST}"
...
# Create admin + publisher users (idempotent — `ntfy user add` errors if exists)
if ! ntfy user list 2>/dev/null | grep -q "^user ${E[NTFY_ADMIN_USER]} "; then
    NTFY_PASSWORD="${E[NTFY_ADMIN_PASSWORD]}" ntfy user add --role=admin "${E[NTFY_ADMIN_USER]}" >/dev/null
fi
if ! ntfy user list 2>/dev/null | grep -q "^user ${E[NTFY_PUBLISH_USER]} "; then
    NTFY_PASSWORD="${E[NTFY_PUBLISH_PASSWORD]}" ntfy user add "${E[NTFY_PUBLISH_USER]}" >/dev/null
    ntfy access "${E[NTFY_PUBLISH_USER]}" "u-*" write >/dev/null
fi

# Sudoers — allow mypa to manage per-user ntfy accounts at runtime
cat > /etc/sudoers.d/mypa-ntfy <<'SUDOERS'
# mypa-api manages per-user ntfy accounts. SETENV: passes NTFY_PASSWORD through.
...
visudo -c -f /etc/sudoers.d/mypa-ntfy >/dev/null

# Caddy block for ntfy (separate site from mypa)
if ! grep -q "^${NTFY_HOST} {" /etc/caddy/Caddyfile 2>/dev/null; then
...
# Re-write env so the new ntfy fields land
{
    echo "# MyPA configuration — generated $(date -u +%FT%TZ) by setup.sh"
    echo "# Mode 0600. Owned by mypa user. Contains secrets — do not commit."
    for key in PUBLIC_HOST USER_NAME TZ LOCALE \
               ...
               OPENAI_API_KEY \
               IMPLICIT_LOCATION_CAPTURE IMAGE_EXTRACTION_ENABLED \
               OUTCOME_NUDGE_ENABLED \
               NTFY_HOST NTFY_BASE_URL NTFY_ADMIN_USER NTFY_ADMIN_PASSWORD \
               NTFY_PUBLISH_USER NTFY_PUBLISH_PASSWORD; do
        echo "$key=${E[$key]:-}"
    done
} > "$ENV_FILE"
chmod 0600 "$ENV_FILE"
...
if systemctl is-active --quiet ntfy && curl -fsS "http://127.0.0.1:9031/v1/health" >/dev/null; then
    color 32 "  ✓ ntfy responding on :9031"
else
    warn "ntfy not responding on :9031 — check 'journalctl -u ntfy'."
fi
...
echo "  OAUTH_JWT_SECRET (signs all OAuth JWTs — LOSE THIS = all sessions invalid):"
echo "    ${E[OAUTH_JWT_SECRET]}"
echo "  NTFY_ADMIN_PASSWORD (for ntfy server admin operations):"
echo "    ${E[NTFY_ADMIN_PASSWORD]}"
echo "  NTFY_PUBLISH_PASSWORD (mypa-api uses this to send pushes):"
echo "    ${E[NTFY_PUBLISH_PASSWORD]}"
echo
echo "  ntfy URL:          https://${E[NTFY_HOST]}/"
echo "  (Each user gets their own ntfy account auto-created on signup;"
echo "   they can fetch their personal credentials from GET /me/notify-prefs.)"
echo
color 32 "================================================================"
echo "  Next steps:"
